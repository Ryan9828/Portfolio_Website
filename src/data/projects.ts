...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description:
      'An LSTM sequence classifier trained on 1.85M credit card transactions, learning a sliding window of each cardholder\'s history rather than classifying transactions in isolation. Threshold tuned against an actual cost model (LexisNexis True Cost of Fraud) instead of F1. Packaged into a FastAPI service, containerised with Docker, and deployed to AWS EC2 for real-time inference.',
    flagship: false,
    accent: 'pink',
    icon: 'ShieldAlert',
  ...
  },
  {
    ...
    description:
      'An investor-focused analysis of LendingClub\'s public lending data, testing whether interest rates suppress loan demand and whether they can be used to maximise investor returns. SARIMAX time-series modelling, logistic regression, and a return-optimisation framework showed the answer to both was no — platform growth and borrower fundamentals (FICO, DTI, credit utilisation) drive outcomes, not rate levels.',
    flagship: false,
    accent: 'yellow',
    icon: 'Landmark',
    ...
    metrics: [
      ...
      { value: '4-tier', label: 'borrower screening framework' },
    ],
    links: {
      github: { label: 'Code coming soon', url: null, status: 'pending' },
    },
  },
  {
    ...
    description:
      'A 25-class IoT device classifier built with KDDI Corporation (Japanese telco), working from 19.12M IPFIX network flow records — metadata only, no traffic content — enabling privacy-preserving device monitoring at ISP scale. The core challenges were severe class imbalance, concept drift across months, and label shift in the test set. Capstone code is not publicly released due to data-sensitivity agreements with KDDI; this is a methodology write-up.',
    flagship: false,
    accent: 'blue',
    icon: 'Cpu',
  ...
  },
  {
    ...
    description:
      'A two-phase AI pipeline automating the graduate job search workflow — daily scraping across 5 job sites, filtered and ranked by relevance, with on-demand per-job research, resume tailoring, verification, and cover letter generation. A conductor agent coordinates 4 specialised subagents, each with a scoped context window, and an automated resume-verification loop scores and re-feeds failures for targeted fixes.',
    flagship: false,
    accent: 'orange',
    icon: 'Bot',
